        ...
        extends PrimitiveValueWriter
{
    ...
    public GeometryValueWriter(Supplier<ValuesWriter> valuesWriterSupplier, Type type, PrimitiveType parquetType)
    {
    ...
    public void write(Block block)
    {
        for (int i = 0; i < block.getPositionCount(); i++) {
            if (!block.isNull(i)) {
                Slice slice = Slices.wrappedBuffer(EsriGeometrySerde.deserialize(type.getSlice(block, i)).asBinary());
